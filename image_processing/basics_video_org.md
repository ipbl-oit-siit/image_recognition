...
| code | comment |
| --- | --- |
| `cap.read()` | 1st return value is a boolean value for whether success in reading a frame <br>

<br> 2nd return value is the list of the pixel values in a frame |
...
* Copy [`video_viewer1.py`](https://www.google.com/search?q=%23video_viewer1py), paste it to VS Code, and save it as `video_viewer1.py` in the `C:\oit\home\ipbl` folder.
...
* Make a "Let's selfie program" (`selfie.py`) by modifying [`video_viewer1.py`](https://www.google.com/search?q=%23video_viewer1py).
...
| code | comment |
| --- | --- |
| `cv2.imwrite("name", variable)` | 1st argument is the file name(path) of the image which is saved. <br>

<br>2nd argument is the variable of the image. |
